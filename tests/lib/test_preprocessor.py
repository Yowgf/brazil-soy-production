...
class TestPreprocessor:
    ...
    @pytest.fixture
    def preprocessor_all(self):
        return Preprocessor("database", "all")
    ...
    def test_preprocessor_get_table_all(self, preprocessor_all):
        tab = preprocessor_all.get_table()
        expected_first_part = pd.DataFrame({
            'year': [1920, 1940],
            'region': ["Acre", "Acre"],
            'production': [0.0, 0.0],
        ...
        actual_first_part = tab.iloc[:2].reset_index(drop=True)
        actual_last_part = tab.iloc[-2:].reset_index(drop=True)
        print(actual_first_part)
        print(expected_first_part)
        print(actual_last_part)
        print(expected_last_part)
        assert actual_first_part.equals(expected_first_part)
        assert actual_last_part.equals(expected_last_part)

        assert "Brasil" in tab.region.unique()
        assert "Sul" in tab.region.unique()
